  ...
  if (millis() - lastSendTime >= sendInterval) {
    ...
    if (newDataReady) {
      if (millis() > t + serialPrintInterval) {
        ...
        // === Kirim ke Firebase ===
        Firebase.RTDB.setInt(&fbdo, "/TS2/KETINGGIAN_SAMPAH_CM", tinggi_sampah);
        Firebase.RTDB.setFloat(&fbdo, "/TS2/VOLUME_PERSEN", persentase_volume);
        Firebase.RTDB.setFloat(&fbdo, "/TS2/LOAD_CELL_WEIGHT", berat);
        Firebase.RTDB.setFloat(&fbdo, "/TS2/MQ2_PPM", ppm);
        ...
        Firebase.RTDB.setString(&fbdo, "/TS2/TRASH_STATUS", status);
